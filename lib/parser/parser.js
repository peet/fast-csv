    ...
    function parseItem(line, items, cursor, hasMoreData) {
        ...
        if (nextChar.search(delimiter) !== -1) {
            if (hasMoreData && (cursor + (nextIndex + 1) >= line.length)) {
                ...
                // if ends with a delimiter, append an empty element
                if ((ROW_DELIMITER.test(cursorChar) || cursor >= line.length)) {
                    items.push('');
                }
                //if ends with empty space that is not a delimiter, append an empty space
                let nextToken = getNextToken(line, cursor).token;
                if (SPACE_CHAR_REGEX.test(cursorChar) && (isUndefinedOrNull(nextToken) || ROW_DELIMITER.test(nextToken))) {
